...
function Page() {
  ...
  useEffect(() => {
    axios
      .get(`https://agial-toys-10.onrender.com/products`)
  ...
  return (
    <div>
      <Container className="text-center mt-5 pt-3">
        <Row>
          <h2 className="text-center mt-5">المنتجات الأكثر مبيعا</h2>
          {products.map((abdo) => (
            <Col md="4" key={abdo.name}>
              <img
                src={abdo.image}
              ...
              <button className="btn btn-dark" style={{ backgroundColor: "darkgreen" }} onClick={() => addItem(abdo)}>
                إضافة إلي السلة
              </button>
              <>
                {values.map((v, idx) => (
